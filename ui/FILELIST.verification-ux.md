...
| `ui/src/adapters/pageImageRefs.ts` | Validates content-addressed PNG references, builds generic and calibration-manifest indexes, resolves mandatory source images, and isolates xywh/xyxy projection. | Annotation-call bboxes are xywh; mature extraction/calibration/retrieval bboxes are xyxy; absolute bboxes need width/height metadata; page indices pass through unchanged. |
...
| `ui/src/adapters/calibration.ts` | Parses `sample_v1.jsonl`, preserves the server’s exact row identity, converts xyxy boxes for presentation, and validates/serializes exact `labels_v1.jsonl` rows. | Quintiles are live integer strata 0–4; sample bboxes are normalized `[x0,y0,x1,y1]`; confidence never enters rendered output. |
...
| `ui/server/index.ts` | Preserves the standalone loop/review API and Codex’s verified calibration sample/label endpoints. | Source PNG bytes and canonical content-addressed identities are checked before sample delivery; label rows must target a known sample item. |
...
| `ui/tests/contracts.test.ts` | Exercises winner adapters against exact annotation, calibration image, and section-path contracts. | Deterministic Node test; no mocks or network. |
| `ui/tests/calibrate.playwright.test.mjs` | Runs the built UI and real API in Chromium; verifies calibration persistence, retrieval evidence, bbox geometry, mandatory page-image failure, and screenshots. | Uses a temporary artifact root with real PNG bytes and canonical hashes; screenshots are written under `/tmp`. |
...
| `ui/package.json` | Adds repeatable Vitest, contract, build, and Playwright commands and their test dependencies. | Runtime React/Vite versions remain unchanged. |
