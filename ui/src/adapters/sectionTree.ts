...
// Compatibility adapter retained for the existing labeling renderer. New
// verification routes use normalizeSectionTree above; this narrow projection
// keeps the archived/salvaged breadcrumb presentation stable.
...
function isFiniteCompatibilityBbox(value: unknown): value is [number, number, number, number] {
...
export function assertSectionTreeV2(value: unknown): asserts value is SectionTreeV2 {
  ...
  if (tree.schema !== SECTION_TREE_SCHEMA) {
    throw new Error(`section tree schema must be ${SECTION_TREE_SCHEMA}`)
  }
  ...
  for (const section of tree.sections) {
    ...
    if (
      ...
      || provenance.page < 0
      || (provenance.bbox !== null && !isFiniteCompatibilityBbox(provenance.bbox))
    ) {
