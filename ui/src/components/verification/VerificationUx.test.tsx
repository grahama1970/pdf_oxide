...
describe('verification-first UX', () => {
  ...
  it('keeps confidence opaque and writes an exact labels_v1 row', async () => {
    const [sample] = parseCalibrationSample(JSON.stringify({
      ...
      label: null,
      page_image_refs: [{ sha256: IMAGE_SHA, href: `/page_images/${IMAGE_SHA}.png`, page: 3 }],
    }))
    const persistLabel = vi.fn(async (_row: CalibrationLabelRow) => undefined)
    ...
    if (!row) throw new Error('expected one persisted labels_v1 row')
