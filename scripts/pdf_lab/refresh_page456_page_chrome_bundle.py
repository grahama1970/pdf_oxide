#!/usr/bin/env python3
"""Materialize a fresh page456 page-chrome evidence bundle."""
...
DEFAULT_TEST = REPO / "tests/test_nist_page456_page_chrome.py"
HISTORICAL_BUG_REPORT = (
    REPO / "artifacts/pdf_lab/scillm_bug_report_pilot_gpt55_clusters/page_0456_page_chrome/bug_report.json"
)
...
def summarize_page456_chrome(extraction: dict[str, Any]) -> dict[str, Any]:
    ...
    top_headers = [
        block
        for block in blocks
        if "NIST SP 800-53" in normalize(block.get("text"))
        and "SECURITY AND PRIVACY CONTROLS" in normalize(block.get("text"))
    ]
    footer_blocks = [
        block
        for block in blocks
        if "APPENDIX C" in normalize(block.get("text")) and "PAGE 429" in normalize(block.get("text"))
    ]
    leaked_chrome = [
        {
            "id": block.get("id"),
        ...
        and (
            "NIST SP 800-53" in normalize(block.get("text"))
            or "APPENDIX C" in normalize(block.get("text"))
            or "PAGE 429" in normalize(block.get("text"))
        )
    ]
    return {
        ...
        "top_header_count": len(top_headers),
        "top_header": top_headers[0] if top_headers else None,
        "footer_page_chrome_count": len(footer_blocks),
        "footer_page_chrome": footer_blocks[0] if footer_blocks else None,
        "semantic_chrome_leak_count": len(leaked_chrome),
        "semantic_chrome_leaks": leaked_chrome,
    }
...
def validate_receipt(receipt: dict[str, Any], extraction_json: Path) -> dict[str, Any]:
    ...
    if summary.get("top_header_count") != 1:
        errors.append("top_header_count_not_one")
    if summary.get("footer_page_chrome_count") != 1:
        errors.append("footer_page_chrome_count_not_one")
    if summary.get("semantic_chrome_leak_count") != 0:
        errors.append("semantic_chrome_leaks_present")
    top_header = summary.get("top_header") or {}
    if top_header.get("type") != "header_footer_noise" or top_header.get("source_type") != "Header":
        errors.append("top_header_wrong_type")
    footer = summary.get("footer_page_chrome") or {}
    if footer.get("type") != "header_footer_noise" or footer.get("source_type") != "Footer":
        errors.append("footer_wrong_type")
    return {
        "ok": not errors,
        "errors": errors,
...
def main() -> int:
    ...
    write_json(bbox_metrics, summarize_page456_chrome(extraction))
    ...
    receipt = {
        "schema": "pdf_oxide.page456_page_chrome_evidence.v1",
        "created_at": utc_now(),
        "run_id": args.run_id,
        "page": 456,
        "pdf": str(args.pdf),
