#!/usr/bin/env python3
"""Materialize a fresh page468 table-duplicate evidence bundle."""
...
DEFAULT_TEST = REPO / "tests/test_nist_page468_table_duplicate_suppression.py"
HISTORICAL_CANDIDATE = (
    REPO / "artifacts/pdf_lab/scillm_bug_report_pilot_gpt55_clusters/page_0468/candidate_bundle.json"
)
HISTORICAL_BUG_REPORT = (
    REPO / "artifacts/pdf_lab/scillm_bug_report_pilot_gpt55_clusters/page_0468_table_duplicate_cells/bug_report.json"
)
...
def extract_page(pdf: Path, ledger: Path) -> tuple[dict[str, Any], Any]:
    ...
    try:
        ...
        return snapshot._extract_page(pdf, 467, ledger, "release"), snapshot
    finally:
...
def summarize_page468(extraction: dict[str, Any], snapshot: Any) -> dict[str, Any]:
    ...
    header = rows[0] if rows and isinstance(rows[0], dict) else {}
...
def main() -> int:
    ...
    width, height = render_page(args.pdf, 468, page_png, args.dpi)
    ...
    write_json(bbox_metrics, summarize_page468(extraction, snapshot))
    ...
    receipt = {
        "schema": "pdf_oxide.page468_table_duplicate_evidence.v1",
        "created_at": utc_now(),
        "run_id": args.run_id,
        "page": 468,
        "pdf": str(args.pdf),
        ...
        "extraction_command": "snapshot_current_extraction._extract_page(pdf, 467, ledger, 'release')",
