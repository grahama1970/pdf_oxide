#!/usr/bin/env python3
"""Materialize a fresh page27 page-chrome evidence bundle."""
...
DEFAULT_TEST = REPO / "tests/test_nist_page27_page_chrome.py"
HISTORICAL_BUG_REPORT = (
    REPO / "artifacts/pdf_lab/scillm_bug_report_pilot_gpt55_clusters/page_0027_page_chrome/bug_report.json"
)
...
def extract_page(pdf: Path, ledger: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return snapshot._extract_page(pdf, 26, ledger, "release")
    finally:
...
def summarize_page27_chrome(extraction: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "running_header_count": sum(1 for block in blocks if block.get("type") == "running_header"),
        "top_header_text_band_count": len(top_headers),
        "top_header_text_band": top_headers[0] if top_headers else None,
        "stale_fragment_count": len(stale_fragments),
...
def validate_receipt(receipt: dict[str, Any], extraction_json: Path) -> dict[str, Any]:
    ...
    if summary.get("top_header_text_band_count") != 1:
        errors.append("top_header_text_band_count_not_one")
    if summary.get("stale_fragment_count") != 0:
        errors.append("stale_running_header_fragments_present")
    if summary.get("running_header_count") != 0:
        errors.append("running_header_blocks_present")
    return {
        "ok": not errors,
        "errors": errors,
...
def main() -> int:
    ...
    write_json(bbox_metrics, summarize_page27_chrome(extraction))
    ...
    receipt = {
        "schema": "pdf_oxide.page27_page_chrome_evidence.v1",
        "created_at": utc_now(),
        "run_id": args.run_id,
        "page": 27,
        "pdf": str(args.pdf),
