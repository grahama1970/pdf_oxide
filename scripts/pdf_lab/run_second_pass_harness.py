...
def validate_commit_gate_acceptance(commit_gate: dict[str, Any] | None) -> dict[str, Any]:
    ...
    if (
        ...
        changed_files = []
    if (
        not isinstance(committed_files, list)
        or not committed_files
        or not all(isinstance(path, str) and path for path in committed_files)
    ):
    ...
    *,
    ...
    for result in page_results:
        ...
        if review_after_request_validation:
            ...
            elif review_after_request_validation.get("errors") not in (None, []):
                entry_errors.append("review_after_request_validation ok is true but errors are not empty")
        ...
        if revertability:
            ...
            elif revertability.get("errors") not in (None, []):
                entry_errors.append("revertability_check ok is true but errors are not empty")
    ...
    *,
    ...
    if expected_schema is not None:
        ...
        if not isinstance(zip_path, str) or not zip_path:
            ...
            if isinstance(included_artifacts, list) and all(isinstance(entry, str) for entry in included_artifacts):
                missing_included_artifacts = sorted(set(included_artifacts) - set(actual_zip_entries))
                if missing_included_artifacts:
                    error_label = label or "package validation"
                    errors.append(
                        f"{error_label} actual ZIP missing included artifacts: {missing_included_artifacts}"
                    )
                undeclared_zip_entries = sorted(set(actual_zip_entries) - set(included_artifacts))
                if undeclared_zip_entries:
                    error_label = label or "package validation"
                    errors.append(f"{error_label} actual ZIP has undeclared entries: {undeclared_zip_entries}")
            ...
            if is_plain_int(reported_zip_entry_count) and reported_zip_entry_count != actual_zip_entry_count:
                error_label = label or "package validation"
                errors.append(
                    f"{error_label} zip_entry_count {reported_zip_entry_count} "
                    f"does not match actual ZIP entry count {actual_zip_entry_count}"
                )
    for key in [
        "included_artifacts",
    ...
    if validation.get("invalid_zip") is True:
        errors.append("invalid_zip is true")
    return errors
...
def validate_harness_page_review_bundle(case_dir: Path, zip_path: Path, terminal: dict[str, Any]) -> dict[str, Any]:
    ...
    if terminal_ledger_path.is_file():
        terminal_ledger_payload, read_errors = read_required_json_object(terminal_ledger_path, "terminal_ledger.json")
        errors.extend(read_errors)
