#!/usr/bin/env python3
"""Materialize a fresh page28 footnote evidence bundle."""
...
DEFAULT_TEST = REPO / "tests/test_nist_page28_footnotes.py"
HISTORICAL_BUG_REPORT = (
    REPO / "artifacts/pdf_lab/scillm_bug_report_pilot_gpt55_clusters/page_0028_footnote/bug_report.json"
)
EXPECTED_FOOTNOTES = [
...
def summarize_page28_footnotes(extraction: dict[str, Any]) -> dict[str, Any]:
    blocks = extraction.get("blocks") or []
    historical_bug_report = load_json_if_present(HISTORICAL_BUG_REPORT)
    target_footnotes: list[dict[str, Any]] = []
    ...
    for expected in EXPECTED_FOOTNOTES:
        matches = [
            block
            for block in blocks
            if normalize(block.get("text")).startswith(expected)
        ]
        if not matches:
            missing_expected.append(expected)
            continue
        target_footnotes.extend(matches)
    for block in blocks:
        if block.get("type") not in {"section_heading", "paragraph_block"}:
            continue
        if any(normalize(block.get("text")).startswith(expected) for expected in EXPECTED_FOOTNOTES):
            leaks.append(
                {
                    "id": block.get("id"),
                    "type": block.get("type"),
    ...
    return {
        ...
        "footnote_count": sum(1 for block in blocks if block.get("type") == "footnote"),
        "target_footnote_count": len(target_footnotes),
        "target_footnotes": target_footnotes,
        "missing_expected_count": len(missing_expected),
        "missing_expected": missing_expected,
        "section_or_paragraph_footnote_leak_count": len(leaks),
        "section_or_paragraph_footnote_leaks": leaks,
    }
...
def validate_receipt(receipt: dict[str, Any], extraction_json: Path) -> dict[str, Any]:
    ...
    if summary.get("target_footnote_count") != len(EXPECTED_FOOTNOTES):
        errors.append("target_footnote_count_mismatch")
    if summary.get("missing_expected_count") != 0:
        errors.append("expected_footnotes_missing")
    if summary.get("section_or_paragraph_footnote_leak_count") != 0:
        errors.append("section_or_paragraph_footnote_leaks_present")
    for block in summary.get("target_footnotes") or []:
        if block.get("type") != "footnote" or block.get("source_type") != "Footnote":
            errors.append("target_footnote_wrong_type")
            break
    return {
        "ok": not errors,
        "errors": errors,
...
def main() -> int:
    ...
    write_json(bbox_metrics, summarize_page28_footnotes(extraction))
    ...
    receipt = {
        "schema": "pdf_oxide.page28_footnote_evidence.v1",
        "created_at": utc_now(),
        "run_id": args.run_id,
        "page": 28,
        "pdf": str(args.pdf),
