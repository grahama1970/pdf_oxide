#!/usr/bin/env python3
"""Materialize a fresh page27 table-duplicate evidence bundle."""
...
DEFAULT_TEST = REPO / "tests/test_nist_page27_table_duplicate_suppression.py"
HISTORICAL_CANDIDATE = (
    REPO / "artifacts/pdf_lab/scillm_bug_report_pilot_gpt55_clusters/page_0027/candidate_bundle.json"
)
HISTORICAL_BUG_REPORT = (
    REPO / "artifacts/pdf_lab/scillm_bug_report_pilot_gpt55_clusters/page_0027_table_duplicate_cells/bug_report.json"
)
...
def extract_page(pdf: Path, ledger: Path) -> tuple[dict[str, Any], Any]:
    sys.path.insert(0, str(REPO / "scripts/pdf_lab"))
    try:
        import snapshot_current_extraction as snapshot  # noqa: PLC0415

        return snapshot._extract_page(pdf, 26, ledger, "release"), snapshot
    finally:
...
def summarize_page27(extraction: dict[str, Any], snapshot: Any) -> dict[str, Any]:
    ...
    for block in blocks:
        if block.get("type") == "table":
            continue
        bbox = block.get("bbox")
        if not isinstance(bbox, list) or len(bbox) != 4 or len(table_bbox) != 4:
            continue
        if snapshot._bbox_coverage(bbox, table_bbox) >= 0.90:
            leaks.append(
                {
                    "id": block.get("id"),
                    "type": block.get("type"),
    ...
    historical_candidate = load_json_if_present(HISTORICAL_CANDIDATE)
    historical_bug_report = load_json_if_present(HISTORICAL_BUG_REPORT)
    return {
        "historical_candidate": str(HISTORICAL_CANDIDATE),
        "historical_bug_report": str(HISTORICAL_BUG_REPORT),
        "historical_fix_error_count": len(historical_candidate.get("fix_error_requests") or []),
        "historical_bug_report_count": len(historical_bug_report.get("bug_reports") or []),
        "block_count": len(blocks),
        "table_count": len(tables),
        "non_table_count": len(blocks) - len(tables),
        "header_footer_noise_count": sum(1 for block in blocks if block.get("type") == "header_footer_noise"),
        "table_contained_leak_count": len(leaks),
        "leaks": leaks,
        "table": {
            "id": table.get("id"),
            "bbox": table_bbox,
            ...
            "table_contained_fragment_count": len(raw.get("table_contained_fragment_ids") or []),
...
def validate_receipt(receipt: dict[str, Any], extraction_json: Path) -> dict[str, Any]:
    ...
    if summary.get("table_contained_leak_count") != 0:
        errors.append("table_contained_leaks_present")
    if int((summary.get("table") or {}).get("table_contained_fragment_count") or 0) <= 0:
        errors.append("table_fragment_lineage_missing")
    return {
        "ok": not errors,
        "errors": errors,
...
def main() -> int:
    ...
    width, height = render_page(args.pdf, 27, page_png, args.dpi)
    extraction, snapshot = extract_page(args.pdf, args.ledger)
    write_json(extraction_json, extraction)
    render_overlay(page_png, extraction.get("blocks") or [], overlay_png)
    write_json(bbox_metrics, summarize_page27(extraction, snapshot))
    ...
    receipt = {
        "schema": "pdf_oxide.page27_table_duplicate_evidence.v1",
        "created_at": utc_now(),
        "run_id": args.run_id,
        "page": 27,
        "pdf": str(args.pdf),
        ...
        "extraction_command": "snapshot_current_extraction._extract_page(pdf, 26, ledger, 'release')",
