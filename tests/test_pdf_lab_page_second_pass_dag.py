...
def test_validate_page_case_identity_rejects_unsafe_or_stale_case_ids() -> None:
    dag = _load_module()
...
def test_run_page_case_rejects_unsafe_case_id_before_writing_case_dir(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert "case_id must match page_case_####_p####" in str(exc)
    else:
        raise AssertionError("unsafe page case id was accepted")
...
def test_validate_page_orchestrator_run_receipt_rejects_unbounded_dry_run_timeout(tmp_path: Path) -> None:
    dag = _load_module()
    page_case = {
        "case_id": "page_case_0001_p0001",
        "page_number": 1,
...
def test_runtime_boolean_inputs_reject_coerced_values() -> None:
    dag = _load_module()

    errors = dag.validate_runtime_boolean_inputs(opencode_cleanup_session="false")

    assert "opencode_cleanup_session must be a boolean: 'false'" in errors


def test_run_page_case_rejects_invalid_boolean_controls_before_extraction(tmp_path: Path) -> None:
    dag = _load_module()
    sampled_cases = {
        "page_cases": [
            {
    ...
    assert "opencode_cleanup_session must be a boolean: 'false'" in message
    assert not (tmp_path / "out/page_case_0001_p0001").exists()
...
def test_validate_review_response_rejects_stale_receipt_payload() -> None:
    dag = _load_module()
    review = {
        "schema": "pdf_lab.second_pass.review_response.v1",
        "page_status": "defect",
    ...
    stale_review = {
        "schema": "pdf_lab.second_pass.review_response.v1",
        "page_status": "clean",
        "page_rationale": "stale clean review",
        "candidate_findings": [
            {
                "candidate_id": "cand:p0003:0000:table",
                "status": "clean",
                "evidence": "stale rendered evidence",
    ...
    assert validation["ok"] is False
    assert "review receipt review_response does not match validated review response" in validation["errors"]
...
def test_validate_page_terminal_ledger_rejects_stale_commit_terminal_flags(tmp_path: Path) -> None:
    dag = _load_module()
    case_dir = tmp_path / "case"
    _write_full_patched_confirmed_artifacts(case_dir)
    (case_dir / "review.html").write_text("review", encoding="utf-8")
    ...
    assert "terminal commit_gate_ok does not match commit_gate.ok" in errors
    assert "terminal commit_exact_file_match does not match commit_gate.exact_file_match" in errors
    assert "terminal commit_revertability_ok does not match revertability_check.ok" in errors
    assert "terminal commit_acceptance_ok does not match commit_acceptance_gate.ok" in errors
...
def test_validate_page_terminal_ledger_rejects_live_after_review_without_receipt_or_fixture(tmp_path: Path) -> None:
    dag = _load_module()
    case_dir = tmp_path / "case"
    _write_full_patched_confirmed_artifacts(case_dir)
    (case_dir / "review.html").write_text("review", encoding="utf-8")
    ...
    (case_dir / "review_after_request.json").write_text(json.dumps(after_request), encoding="utf-8")
    (case_dir / "review_after_request_validation.json").write_text(
        json.dumps(dag.validate_review_request_contract(case_dir, after_request)),
        encoding="utf-8",
    )
    terminal = {
        "schema": "pdf_lab.second_pass.page_terminal_ledger.v1",
    ...
    assert validation["ok"] is False
    assert (
        "review_after_validation ok true requires scillm_after_review_receipt.json or review_after_fixture.json evidence"
        in validation["errors"]
    )
...
def test_validate_page_terminal_ledger_rejects_coerced_patch_attempt_ok(tmp_path: Path) -> None:
    dag = _load_module()
    case_dir = tmp_path / "case"
    case_dir.mkdir()
    (case_dir / "review.html").write_text("review", encoding="utf-8")
    ...
    assert validation["ok"] is False
    assert "patch_attempts_ledger attempts[0].ok must be boolean" in validation["errors"]


def test_validate_page_terminal_ledger_rejects_string_patch_attempt_errors(tmp_path: Path) -> None:
    dag = _load_module()
    case_dir = tmp_path / "case"
    case_dir.mkdir()
    (case_dir / "review.html").write_text("review", encoding="utf-8")
...
def test_validate_page_terminal_ledger_rejects_unsafe_patch_attempt_artifact_paths(tmp_path: Path) -> None:
                ...
                "attempts": [
                    {
                        "attempt_index": 1,
                        "agent": "build",
                        "validation_artifact": "../outside_validation.json",
                        "request_artifact": "../outside_request.json",
                        "receipt_artifact": "/tmp/outside_receipt.json",
                        "ok": True,
                        "errors": [],
                    }
                ],
    ...
    assert validation["ok"] is False
    errors = "\n".join(validation["errors"])
    assert "patch_attempts_ledger attempts[0].validation_artifact contains unsafe artifact path" in errors
    assert "patch_attempts_ledger attempts[0].request_artifact contains unsafe artifact path" in errors
    assert "patch_attempts_ledger attempts[0].receipt_artifact contains unsafe artifact path" in errors
    assert not (case_dir.parent / "outside_validation.json").exists()
...
def test_validate_page_terminal_ledger_rejects_successful_repair_plan_without_receipt(tmp_path: Path) -> None:
    dag = _load_module()
    case_dir = tmp_path / "case"
    case_dir.mkdir()
    (case_dir / "review.html").write_text("review", encoding="utf-8")
    ...
    assert validation["ok"] is False
    errors = "\n".join(validation["errors"])
    assert "repair_plan_validation ok true requires repair_plan_request.json evidence" in errors
...
def test_validate_page_terminal_ledger_rejects_successful_repair_diagnosis_without_receipt(tmp_path: Path) -> None:
    dag = _load_module()
    case_dir = tmp_path / "case"
    case_dir.mkdir()
    (case_dir / "review.html").write_text("review", encoding="utf-8")
...
def test_validate_page_terminal_ledger_rejects_string_review_validation_errors(tmp_path: Path) -> None:
    dag = _load_module()
    case_dir = tmp_path / "case"
    case_dir.mkdir()
    (case_dir / "review.html").write_text("review", encoding="utf-8")
    ...
    assert validation["ok"] is False
    errors = "\n".join(validation["errors"])
    assert "review_validation errors must be a list" in errors
    assert "dry_run_review_not_executed terminal ledger requires matching review_validation error" in errors
...
def test_validate_page_terminal_ledger_rejects_opencode_preflight_without_base_health(tmp_path: Path) -> None:
    dag = _load_module()
    case_dir = tmp_path / "case"
    case_dir.mkdir()
    (case_dir / "review.html").write_text("review", encoding="utf-8")
    ...
    stale_serve_checks = [
        ...
        {"path": "/v1/scillm/opencode/health", "http_status": 200, "payload": {"opencode_serve": True}},
    ]
    (case_dir / "scillm_patch_preflight.json").write_text(
        json.dumps(
            {
    ...
    assert validation["ok"] is False
    errors = "\n".join(validation["errors"])
    assert "scillm_patch_preflight.json ok true requires /v1/scillm/health check" in errors
...
def test_validate_page_terminal_ledger_rejects_malformed_page_identity(tmp_path: Path) -> None:
    dag = _load_module()
    case_dir = tmp_path / "case"
    case_dir.mkdir()
    (case_dir / "review.html").write_text("review", encoding="utf-8")
...
def test_validate_page_review_bundle_rejects_duplicate_terminal_evidence(tmp_path: Path) -> None:
    dag = _load_module()
    case_dir = tmp_path / "case"
    case_dir.mkdir()
    for name in sorted(dag.MINIMUM_PAGE_REVIEW_BUNDLE_ARTIFACTS):
    ...
    (case_dir / "terminal_ledger_validation.json").write_text(
        json.dumps(dag.validate_page_terminal_ledger(case_dir, terminal)),
        encoding="utf-8",
    )
    zip_path = case_dir / "review_bundle.zip"
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as bundle:
